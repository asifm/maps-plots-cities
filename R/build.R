...
tufte_hugo_html <- function(..., margin_references = TRUE) {
  ...
  format$post_processor <- function(metadata, input, output, clean = FALSE, verbose = TRUE) {
...
tufte_html_page <- function(..., number_sections = FALSE, self_contained = FALSE,
  ...
  rmarkdown::output_format(
    ...
    base_format = tufte_hugo_html(
      ...
      # template = "C:/Users/mehedia/Documents/R/win-library/3.5/rmarkdown/rmd/h/default.html"
    )
  )
}
...
build_rmds <- function(files) {
  ...
  for (f in files) {
    ...
    copy_output_yml(d)
    message("Rendering ", f)
    rmarkdown::render(f, tufte_html_page(), envir = globalenv(), quiet = FALSE,
