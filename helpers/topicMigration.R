...
sql_migrationflow <-
  qry_sql("SELECT * FROM msa_msa_migrationflow_acs12_16")
...
migflow <- migflow %>%
  mutate(flow_perc_sourcepop = flow_prev_to_current / sourcemsa_pop)

migflow %>%
  top_n(20, flow_perc_sourcepop)

msa_5mplus <- filter(migflow, targetmsa_pop > 5000000) %>%
  select(targetmsa_code13) %>%
  distinct()

migflow_bigmsas <- migflow %>%
  filter((targetmsa_code13 %in% msa_5mplus[[1]]) &
           (sourcemsa_code13 %in% msa_5mplus[[1]]))


q1 <- ggplot(
  migflow_bigmsas,
  aes(y = targetmsa_code13, x = sourcemsa_code13, fill = flow_prev_to_current)
) +
  geom_tile()
